...
import { EarthSituationPage } from './components/public/EarthSituationPage';
...
export default function App() {
  const [activeTab, setActiveTab] = useState<AppTabType>('earth-situation');
  ...
  return (
    <RegistryProvider>
      <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
        ...
        {/* Main Content Area */}
        <main className={`flex-1 w-full ${activeTab === 'earth-situation' ? '' : 'max-w-7xl mx-auto p-4 sm:p-6 lg:p-8'}`}>
          {activeTab === 'earth-situation' && (
            <EarthSituationPage
              onNavigateToExpertTab={(tab) => setActiveTab(tab as AppTabType)}
            />
          )}
